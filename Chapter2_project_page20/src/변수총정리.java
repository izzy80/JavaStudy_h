 ...
 * 		.class (컴파일된 파일 : 0,1만들어진 바이너리 파일 => 컴퓨터만 알 수 있는 언어)
 * 	Hello.java =======> Hello.class ======> JVM(main을 번역) ==> 화면  
 * 				javac               java
 * 	
 * 	2. 변수 : 데이터관리 프로그램 (메모리 안에 한 개의 데이터를 저장하는 공간)
 * 	3. 변수명 설정 방법
 * 		1) 알파벳, 한글 시작한다. (알파벳은 대소문자 구분한다)
 * 		2) 숫자사용 가능 (앞에서 사용 금지)
 * 		3) 특수 문자 (_, $) ==> $abc, _abc. main_class, main$class 위치는 상관 없음.
 * 			==> $는 사용빈도가 적다. _앞에, 뒤에  ==> 임시 파일, 임시변수 
 ...
 * 			long(64bit = 8byte) 
 * 			==> int, long 구분시에 접미어를 사용 (L,l) ==> 100L(8byte), 100(4byte)
 * 		실수
 * 			double(8byte ==> 소수점 15자리)
 * 		문자
 * 			char(2byte ==> unsigned (음수가 없는 상태. 0~65535) => 모든 문자에는 정수값을 사용
 ...
 * 			int i =10;
 * 					'A'
 * 					10L(X) -> int보다 커서 사용 불가
 * 					10.5(X)
 * 					10.5F(x)
 * 						그래도 난 쓰고 싶다!!! -> int(10.5) 강제 형변환
 * 
 * 			크기
