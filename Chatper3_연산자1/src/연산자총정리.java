 ...
 * 		4. 연산자  (87page)
 * 		단항연산자 
 * 			***= 증감연산자 (++, --) = 전치연산자(++a), 후지연산자(a++) ==> 1개 증가하는 것은 동일(93page)
 * 				int a = 10;
 * 				int b = ++a; ==> ++a, b = a
 * 
 * 				int a = 10;
 ...
 * 			***= 형변환 연산자 : (type) ==> (int), (char), (double) # (double)은 안 됨. ==> 82page
 * 					*** 작은 데이터형으로 변환 : 오버플로우
 ...
 * 		이항연산자 : 
 * 			***= 산술연산자(+, -, *, / , %) ==> 형변환
 * 			= 비트연산자(&, |, ^) => 내일만 2진법으로 바꿔서 계산하는 방법이~~
 * 			= 쉬트프연산자 (<<, >>) => 내일만
 * 			***= 비교연산자(==, !=, <,>,<=,>=) ==> 결과값이 (boolean) => 조건문
 * 			***= 논리 연산자(&&, ||) => 효율적인 연산
 * 			***= 대입 연산자 =, +=, -=, *=, /=, %=, &=, |=, ^=
 * 					   -----------
 * 		삼항연산자 : 
 * 				(조건)?값1:값2
 * 				조건이 맞으면 앞에 값1, 틀리면 값2를 가져와라 => 페이징(페이지 나눌때 많이 등장)
 * 				if()
 * 				{
 * 				}
 * 				else
 * 				{
 * 				}
 * 증감연산자. 부정연산자. 형변환연산자. 산술연산자. 비교연산자. 논리연산자. 대입연산자 외우기
 * 
 * 
 * 
 * 
