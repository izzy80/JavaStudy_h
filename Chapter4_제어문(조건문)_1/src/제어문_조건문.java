 ...
 * 	2) 필요시에는 건너뛰고, 같은 문장일 경우에는 반복해서 수행 => 이런 것을 제어하는 프로그램(제어문)
 * 					-------------------------반복문(+반복에서 제어)
 * 		-------------조건문 
 * 	3) 제어문 종류
 * 		조건에 필요한 연산자(true/false) => 부정연산자(!), 비교연산자(==,!=,<,>,<=,>=),
 * 																논리연산자(&&,||)
 * 		1. 조건문
 * 		= 단일 조건문
 * 			형식)
 ...
 * 				=> 오류 체크 多 사용(코테에서 중요)
 * 				
 * 				ex)1. 문자열을 입력을 받아서 좌우 대칭인지 여부 확인
 * 					=> 글자수 짝수
 * 					String msg="";
 * 					if (msg.length()%2==0)
 * 					{
 ...
 * 
 * 				   2. 점수를 입력받아서 총점, 평균
 * 					kor ==> if(kor>=0 && kor <=100) ==> if문은 처리가 어려운 부분이 있다.그래서 예외처리(=8장) 사용 
 * 					=> 항상 견고한 프로그램(에러 없는)을 짜기 위해 사용.
 * 				
 * 				
 * 			= 선택 조건문 (가장 많이 사용) => 줄인 형식이 "삼항연산자"
 * 			형식)
 * 				if(조건)
 * 				{
 * 					조건 true일때 수행하는 문장
